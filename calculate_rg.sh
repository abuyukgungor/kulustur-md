#!/bin/bash
# MD Analysis - Radius of Gyration (Rg) Calculation
# Usage: ./calculate_rg.sh <topology> <traj_list> <out_file>
...
# Calculate Radius of Gyration (mass-weighted is default and physically standard)
echo "--> Calculating Radius of Gyration (Rg) for $MASK atoms..."
cat <<EOF > "$OUT_DIR/${OUT_BASE}.in"
parm $TOPOLOGY
$TRAJIN_LINES
radgyr rg_out $MASK out $OUT_FILE
EOF

cpptraj -i "$OUT_DIR/${OUT_BASE}.in" > "$OUT_DIR/cpptraj_${OUT_BASE}.log" 2>&1
if [ $? -ne 0 ]; then
    echo "ERROR: Radius of Gyration calculation failed! Details below:"
...
echo "--> Radius of Gyration calculation completed: $OUT_FILE"
